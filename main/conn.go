...
func (this *Conn) WriteTextMessage(msg []byte) {
	...
	this.wsConn.WriteMessage(ws.TextMessage, msgObjStr)
}
...
func (this *Conn) ReadMessage() (int, []byte, error) {
	...
	if err != nil {
		log.Printf("wsConn %d's error is %s", this.GetIndex(), err.Error())
		return 0, nil, nil
	}
...
func (this *Conn) startListen() {
	...
	for {
		...
		case ws.TextMessage:
			log.Printf("%s", msg)
			msgObj := ParseMessage(msg)
			msgObj.MessageType = ws.TextMessage
			msgObj.From = this.GetIndex()
